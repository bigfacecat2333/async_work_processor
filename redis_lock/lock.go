...
func (f *FixIntervalRetry) Next() (time.Duration, bool) {
	f.cnt++ // 如果cnt大于Max，那么返回false, 不再续约
	return f.Interval, f.cnt <= f.Max
}

// Client
// singleflight.Group
// SingleFlight的作用是在处理多个goroutine同时调用同一个函数的时候，只让一个goroutine去实际调用这个函数，
// 等到这个goroutine返回结果的时候，再把结果返回给其他几个同时调用了相同函数的goroutine，这样可以减少并发调用的数量
type Client struct {
...
func NewClient(client redis.Cmdable) *Client {
	return &Client{
		client: client,
		valuer: func() string {
			return uuid.New().String() // 生成一个uuid
		},
	}
}
...
func (c *Client) Lock(ctx context.Context, key string, expiration time.Duration, retry RetryStrategy, timeout time.Duration) (*Lock, error) {
	...
	for {
		// context.WithTimeout 返回一个新的 Context，该 Context 在 timeout 过后过期。
